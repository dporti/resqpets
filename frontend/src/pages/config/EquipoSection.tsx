    ...
    <div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr>{['', 'Nombre', 'Email', 'Rol', 'Estado', 'Último acceso', 'Acciones'].map(h =>
                <th key={h} style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 600, color: 'var(--text-muted)', borderBottom: '1px solid var(--border)', fontSize: 12, background: 'var(--bg-subtle)' }}>{h}</th>
              )}</tr>
            </thead>
            <tbody>
              {members.map(m => (
                <tr key={m.id}>
                  ...
                  <td style={{ padding: '10px 12px', borderBottom: '1px solid var(--border-subtle)', fontWeight: 600, color: 'var(--text-primary)' }}>{m.nombre}</td>
                  <td style={{ padding: '10px 12px', borderBottom: '1px solid var(--border-subtle)', color: 'var(--text-muted)' }}>{m.email}</td>
                  <td style={{ padding: '10px 12px', borderBottom: '1px solid var(--border-subtle)' }}>
                  ...
                  <td style={{ padding: '10px 12px', borderBottom: '1px solid var(--border-subtle)' }}>
                    <button onClick={() => toggleStatus(m.id)} style={{
                      ...
                      background: m.activo ? '#f0fdf4' : 'var(--bg-subtle)', color: m.activo ? '#16a34a' : '#9ca3af',
                    }}>
                      {m.activo ? '● Activo' : '○ Inactivo'}
                    </button>
                  </td>
                  <td style={{ padding: '10px 12px', borderBottom: '1px solid var(--border-subtle)', color: 'var(--text-faint)', fontSize: 12 }}>
      ...
      {invitations.length > 0 && (
        <SectionCard title="Invitaciones pendientes">
          {invitations.map(inv => (
            <div key={inv.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--border-subtle)' }}>
              <div>
                <p style={{ margin: 0, fontWeight: 600, fontSize: 13, color: 'var(--text-primary)' }}>{inv.email}</p>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => navigator.clipboard.writeText(inv.token)} style={{ padding: '4px 10px', border: '1.5px solid var(--border)', borderRadius: 6, background: 'var(--bg-surface)', cursor: 'pointer', fontSize: 11 }}>📋 Copiar link</button>
      ...
      <SectionCard title="Matriz de permisos" description="Los permisos están definidos por rol y no son modificables individualmente">
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>
                <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: 'var(--text-muted)', borderBottom: '1px solid var(--border)' }}>Acción</th>
                {['Admin','Coordinador','Voluntario'].map(r => (
                  <th key={r} style={{ padding: '8px 12px', textAlign: 'center', fontWeight: 700, color: ROL_COLOR[r.toLowerCase()] || '#374151', borderBottom: '1px solid var(--border)' }}>{r}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {PERMISOS_MATRIX.map(([action, admin, coord, vol]) => (
                <tr key={action}>
                  <td style={{ padding: '7px 12px', borderBottom: '1px solid var(--border-subtle)', color: 'var(--text-secondary)' }}>{action}</td>
                  {[admin, coord, vol].map((v, i) => (
                    <td key={i} style={{ padding: '7px 12px', textAlign: 'center', borderBottom: '1px solid var(--border-subtle)' }}>
                      <span style={{ color: v ? '#16a34a' : 'var(--text-faint)', fontSize: 16, fontWeight: 700 }}>{v ? '✓' : '✗'}</span>
                    </td>
                  ))}
                </tr>
              ))}
      ...
      {showInviteModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div style={{ background: 'var(--bg-surface)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 440, boxShadow: '0 20px 50px rgba(0,0,0,.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>Invitar miembro</h3>
              <button onClick={() => { setShowInviteModal(false); setInvLink(''); }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, color: 'var(--text-faint)' }}>✕</button>
            </div>
            {invLink ? (
              <div>
                <p style={{ color: '#16a34a', fontWeight: 600, marginBottom: 12 }}>✓ Invitación creada</p>
                <div style={{ background: 'var(--bg-subtle)', borderRadius: 8, padding: '10px 12px', fontSize: 12, wordBreak: 'break-all', marginBottom: 16, color: 'var(--text-secondary)' }}>{invLink}</div>
              ...
              <>
                <div style={{ marginBottom: 12 }}>
                  ...
                  <input type="email" value={invEmail} onChange={e => setInvEmail(e.target.value)} style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid var(--border)', fontSize: 13, boxSizing: 'border-box' }} />
                </div>
                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Rol</label>
                  <select value={invRol} onChange={e => setInvRol(e.target.value)} style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid var(--border)', fontSize: 13, background: 'var(--bg-surface)' }}>
                    <option value="voluntario">Voluntario</option>
                    <option value="coordinador">Coordinador</option>
                    <option value="admin">Admin</option>
                  </select>
                ...
                <div style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Mensaje personalizado (opcional)</label>
                  <textarea value={invMsg} onChange={e => setInvMsg(e.target.value)} style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid var(--border)', fontSize: 13, resize: 'vertical', minHeight: 60, boxSizing: 'border-box', fontFamily: 'inherit' }} />
                </div>
