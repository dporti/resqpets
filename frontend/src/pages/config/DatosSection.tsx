  ...
  return (
    <div>
      ...
      <SectionCard title="Política de retención de datos">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            ...
            <div key={item} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border-subtle)', fontSize: 13 }}>
              <span style={{ color: 'var(--text-secondary)' }}>{item}</span>
              <span style={{ fontWeight: 600, color: 'var(--text-muted)' }}>{period}</span>
            </div>
          ))}
        </div>
      </SectionCard>
      ...
      <SectionCard title={`Registro de auditoría (${auditTotal} entradas)`}>
        {loading ? <p style={{ color: 'var(--text-faint)', fontSize: 13 }}>Cargando...</p> : (
          <>
            <div style={{ maxHeight: 320, overflowY: 'auto' }}>
              {auditLog.length === 0 && <p style={{ color: 'var(--text-faint)', fontSize: 13 }}>Sin entradas en el log</p>}
              {auditLog.map(e => (
                <div key={e.id} style={{ display: 'flex', gap: 12, padding: '10px 0', borderBottom: '1px solid var(--border-subtle)', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: 13, color: 'var(--text-primary)' }}>
                      ...
                      {e.resource_name && <span style={{ color: 'var(--text-muted)' }}> en {e.resource_name}</span>}
                    </p>
            ...
            {auditLog.length > 0 && (
              ...
              }} style={{ marginTop: 12, padding: '7px 16px', border: '1.5px solid var(--border)', borderRadius: 8, background: 'var(--bg-surface)', cursor: 'pointer', fontSize: 12, color: 'var(--text-secondary)' }}>
      ...
      {deleteModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--bg-surface)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 440 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => { setDeleteModal(false); setDeleteConfirm(''); }} style={{ flex: 1, padding: '10px', background: 'var(--bg-subtle-2)', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>Cancelar</button>
