...
function ScoreBar({ score }: { score: number }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <div style={{ width: 60, height: 5, background: 'var(--bg-subtle-2)', borderRadius: 3 }}>
...
function KanbanCard({ solicitud, onClick }: { solicitud: AdoptionRequest; onClick: () => void }) {
  ...
  return (
    <div ref={setNodeRef} {...listeners} {...attributes} onClick={onClick}
      style={{
        background: 'var(--bg-surface)', borderRadius: 9, border: '1px solid var(--border)', padding: '11px 13px',
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6 }}>
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{solicitud.nombre}</div>
          <div style={{ fontSize: 11, color: 'var(--text-faint)' }}>{solicitud.animal_nombre}</div>
        </div>
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 11, color: 'var(--text-faint)' }}>{formatDate(solicitud.created_at)}</span>
...
function KanbanColumn({ estado, solicitudes, onCardClick, onDrop }: {
  ...
  return (
    <div ref={setNodeRef} style={{ flex: '0 0 200px', minHeight: 400, background: isOver ? '#f0fdf4' : '#f9fafb', borderRadius: 10, padding: '12px 10px', border: `1px solid ${isOver ? '#86efac' : 'var(--border)'}`, transition: 'all 0.15s' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 12 }}>
        ...
        <span style={{ fontSize: 11, color: 'var(--text-faint)', fontWeight: 600 }}>{solicitudes.length}</span>
      </div>
      {solicitudes.map(s => (
        <KanbanCard key={s.id} solicitud={s} onClick={() => onCardClick(s.id)} />
      ))}
      {solicitudes.length === 0 && (
        <div style={{ textAlign: 'center', color: 'var(--text-faint)', fontSize: 12, padding: '20px 0' }}>Sin solicitudes</div>
      )}
    </div>
  );
}
    ...
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: 'var(--bg-surface)', borderRadius: 14, width: 540, maxHeight: '90vh', overflowY: 'auto', zIndex: 70, fontFamily: "'Inter', sans-serif" }}>
        ...
        <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button onClick={onClose} style={{ padding: '8px 16px', border: '1px solid var(--border)', borderRadius: 7, background: 'var(--bg-surface)', cursor: 'pointer', fontFamily: "'Inter', sans-serif" }}>Cancelar</button>
...
export default function AdopcionesPage() {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: 'var(--bg-subtle)', minHeight: '100vh' }}>
      ...
      {/* Sub-nav */}
      <div style={{ padding: '0 24px', borderBottom: '1px solid var(--border)', background: 'var(--bg-surface)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex' }}>
          {(['solicitudes', 'expedientes'] as const).map(v => (
            <button key={v} onClick={() => { setVista(v); setEstadoFilter(''); }} style={{
              ...
              color: vista === v ? '#16a34a' : 'var(--text-muted)', cursor: 'pointer',
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {(['tabla', 'kanban'] as const).map(m => (
              <button key={m} onClick={() => setViewMode(m)} style={{
                padding: '5px 12px', border: '1px solid var(--border)', borderRadius: 6, fontSize: 12.5,
                background: viewMode === m ? '#111827' : 'var(--bg-surface)', color: viewMode === m ? '#fff' : 'var(--text-muted)',
      ...
      {vista === 'solicitudes' && viewMode === 'tabla' && (
        <div style={{ padding: '12px 24px', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <button onClick={() => setEstadoFilter('')} style={{ padding: '5px 12px', borderRadius: 20, border: '1px solid', borderColor: estadoFilter === '' ? '#16a34a' : 'var(--border)', background: estadoFilter === '' ? '#16a34a' : '#fff', color: estadoFilter === '' ? '#fff' : '#6b7280', fontSize: 12.5, fontWeight: estadoFilter === '' ? 600 : 400, cursor: 'pointer', fontFamily: "'Inter', sans-serif" }}>Todos</button>
          {ESTADOS.map(e => (
            <button key={e.val} onClick={() => setEstadoFilter(e.val)} style={{ padding: '5px 12px', borderRadius: 20, border: '1px solid', borderColor: estadoFilter === e.val ? e.color : 'var(--border)', background: estadoFilter === e.val ? e.bg : '#fff', color: estadoFilter === e.val ? e.color : '#6b7280', fontSize: 12.5, fontWeight: estadoFilter === e.val ? 600 : 400, cursor: 'pointer', fontFamily: "'Inter', sans-serif" }}>{e.label}</button>
          ))}
        </div>
      )}
                  ...
                  <thead>
                    <tr style={{ background: 'var(--bg-subtle)' }}>
                      {['Solicitante', 'Animal', 'Fecha', 'Canal', 'Estado', 'Compatibilidad', ''].map((h, i) => (
                        <th key={i} style={{ padding: '10px 16px', textAlign: 'left', fontSize: 12, color: 'var(--text-muted)', fontWeight: 600, borderBottom: '1px solid var(--border-subtle)' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      >
                        <td style={{ padding: '11px 16px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                            ...
                            <div>
                              <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text-primary)' }}>{s.nombre}</div>
                              <div style={{ fontSize: 11.5, color: 'var(--text-faint)' }}>{s.email || s.telefono || '—'}</div>
                            </div>
                          </div>
                        </td>
                        <td style={{ padding: '11px 16px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{s.animal_nombre || '—'}</span>
                          </div>
                        </td>
                        <td style={{ padding: '11px 16px', fontSize: 12.5, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{formatDate(s.created_at)}</td>
                        <td style={{ padding: '11px 16px' }}>
                          <span style={{ fontSize: 12, color: 'var(--text-muted)', background: 'var(--bg-subtle-2)', padding: '2px 8px', borderRadius: 4, textTransform: 'capitalize' }}>{s.canal || 'web'}</span>
                        </td>
                        <td style={{ padding: '11px 16px' }}><EstadoBadge estado={s.estado} /></td>
                        <td style={{ padding: '11px 16px' }}>
                          {(s.puntuacion || 0) > 0 ? <ScoreBar score={s.puntuacion || 0} /> : <span style={{ color: 'var(--text-faint)', fontSize: 12 }}>—</span>}
                        </td>
                        <td style={{ padding: '11px 16px' }}>
                          <button onClick={e => { e.stopPropagation(); setSelectedSolicitudId(s.id); }} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 10px', fontSize: 12, cursor: 'pointer', color: 'var(--text-secondary)', fontFamily: "'Inter', sans-serif" }}>
                            Ver
                          </button>
                        </td>
                      </tr>
              ...
              {expedientes.map(exp => {
                ...
                return (
                  <div key={exp.id} onClick={() => setSelectedExpedienteId(exp.id)}
                    style={{ background: 'var(--bg-surface)', borderRadius: 12, border: '1px solid var(--border)', padding: '16px 20px', cursor: 'pointer', display: 'flex', gap: 16, alignItems: 'center', transition: 'box-shadow 0.15s' }}
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-primary)' }}>{exp.animal_nombre}</div>
                      <div style={{ fontSize: 12.5, color: 'var(--text-muted)' }}>→ {exp.adoptante_nombre}</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                        <div style={{ flex: 1, background: 'var(--bg-subtle-2)', borderRadius: 4, height: 5 }}>
                          <div style={{ width: `${pct}%`, background: '#16a34a', height: '100%', borderRadius: 4, transition: 'width 0.4s' }} />
                        </div>
                        <span style={{ fontSize: 11.5, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{doneItems}/{totalItems} completados</span>
                      </div>
                    </div>
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-faint)' }}>{diasEnFase(exp)}d en esta fase</div>
                    </div>
