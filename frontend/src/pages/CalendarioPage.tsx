      ...
      <div style={{
        ...
        <div style={{ padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {EVENT_TYPES.map(t => (
                <button key={t.id} onClick={() => !isAutoGen && setType(t.id)} style={{
                  ...
                  background: type === t.id ? t.color + '15' : 'var(--bg-surface)',
          ...
          <div>
            ...
            <select value={animalId} onChange={e => setAnimalId(Number(e.target.value) || '')} style={{ ...inp, background: 'var(--bg-surface)' }}>
              ...
              {users.map(u => {
                ...
                return (
                  <button key={u.id} onClick={() => toggleAssignee(u.id)} style={{
                    ...
                    borderColor: sel ? '#16a34a' : 'var(--border)', background: sel ? 'var(--bg-hover)' : 'var(--bg-surface)',
                    color: sel ? '#16a34a' : 'var(--text-secondary)', cursor: 'pointer', fontSize: 12, fontWeight: sel ? 700 : 400,
                  }}>
                    {sel ? '✓ ' : ''}{u.nombre}
                  </button>
                );
          ...
          <div>
            ...
            <select value={reminder} onChange={e => setReminder(Number(e.target.value) || '')} style={{ ...inp, background: 'var(--bg-surface)' }}>
          ...
          {isRecurring && (
            <select value={recRule} onChange={e => setRecRule(e.target.value)} style={{ ...inp, background: 'var(--bg-surface)' }}>
        ...
        <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border-subtle)', display: 'flex', gap: 10 }}>
          ...
          <div style={{ flex: 1 }} />
          <button onClick={onClose} style={{ padding: '10px 18px', background: 'var(--bg-subtle-2)', color: 'var(--text-secondary)', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>Cancelar</button>
          {!isAutoGen && (
...
const inp: React.CSSProperties = { width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid var(--border)', fontSize: 13, boxSizing: 'border-box', fontFamily: 'inherit' };
...
export default function CalendarioPage() {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--bg-surface)' }}>

      {/* ── LEFT SIDEBAR ── */}
      <div style={{ width: 220, flexShrink: 0, borderRight: '1px solid var(--border)', overflowY: 'auto', background: '#fafafa', padding: '16px 12px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {/* Filtros de tipo */}
        <div>
          <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-faint)', letterSpacing: '.5px', textTransform: 'uppercase', margin: '0 0 8px' }}>Tipos de evento</p>
          {EVENT_TYPES.map(t => (
            <label key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, cursor: 'pointer', fontSize: 13 }}>
              <div style={{
                width: 14, height: 14, borderRadius: 3,
                background: activeTypes[t.id] ? t.color : 'var(--bg-subtle-2)',
                border: `2px solid ${t.color}`, flexShrink: 0, cursor: 'pointer',
              }} onClick={() => toggleType(t.id)} />
              <span style={{ color: activeTypes[t.id] ? 'var(--text-primary)' : 'var(--text-faint)' }}>{t.label}</span>
            </label>
          ))}
        ...
        {/* Próximos eventos */}
        <div>
          <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-faint)', letterSpacing: '.5px', textTransform: 'uppercase', margin: '0 0 8px' }}>Próximos 7 días</p>
          {upcoming.length === 0 && <p style={{ fontSize: 12, color: 'var(--text-faint)' }}>Sin eventos próximos</p>}
          {upcoming.map(e => (
            <div key={e.id} onClick={() => setModal({ open: true, event: e })} style={{
              ...
              <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: 'var(--text-primary)', lineHeight: 1.3 }}>{e.title}</p>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        {/* Toolbar */}
        <div style={{
          padding: '10px 20px', background: 'var(--bg-surface)', borderBottom: '1px solid var(--border)',
          ...
          <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', flex: 1 }} id="cal-title" />
          {/* Views */}
          <div style={{ display: 'flex', gap: 2, background: 'var(--bg-subtle-2)', padding: 3, borderRadius: 8 }}>
            {VIEW_OPTIONS.map(v => (
              <button key={v.id} onClick={() => changeView(v.id)} style={{
                ...
                color: view === v.id ? '#16a34a' : 'var(--text-muted)',
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '0 4px 4px' }}>
          <style>{`
            ...
            .fc-col-header-cell { background: var(--bg-subtle); font-weight: 600; }
          ...
          {loading ? (
            <div style={{ padding: 24 }}>
              <div style={{ height: 500, background: 'linear-gradient(90deg,var(--border-subtle) 25%,var(--border) 50%,var(--border-subtle) 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite', borderRadius: 12 }} />
            </div>
          ) : error ? (
...
const navBtn: React.CSSProperties = {
  padding: '6px 10px', background: 'var(--bg-subtle-2)', border: '1px solid var(--border)',
  borderRadius: 7, cursor: 'pointer', fontSize: 16, color: 'var(--text-secondary)',
};
