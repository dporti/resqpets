          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16 }}>
              <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: '18px 20px' }}>
                <p style={{ margin: '0 0 4px', fontSize: 12, color: '#9ca3af', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.4px' }}>Gastos este mes</p>
                <p style={{ margin: 0, fontSize: 26, fontWeight: 800, color: '#dc2626' }}>{fmt(summary.this_month.expenses)}</p>
              </div>
              <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: '18px 20px' }}>
                <p style={{ margin: '0 0 4px', fontSize: 12, color: '#9ca3af', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.4px' }}>Ingresos este mes</p>
                <p style={{ margin: 0, fontSize: 26, fontWeight: 800, color: '#16a34a' }}>{fmt(summary.this_month.income)}</p>
              </div>
              <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: '18px 20px' }}>
                <p style={{ margin: '0 0 4px', fontSize: 12, color: '#9ca3af', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.4px' }}>Balance este mes</p>
            ...
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: '20px 24px' }}>
              <h3 style={{ margin: '0 0 16px', fontSize: 14, fontWeight: 700, color: '#374151' }}>Gastos por categoría (este año)</h3>
              {summary.by_category.length === 0 ? <p style={{ color: '#9ca3af', fontSize: 13 }}>Sin datos</p> : (
            ...
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: '20px 24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <h3 style={{ margin: 0, fontSize: 14, fontWeight: 700, color: '#374151' }}>Gastos registrados ({gastosTotal})</h3>
              ...
              {gastosPages > 1 && (
                <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 16 }}>
                  <button disabled={page <= 1} onClick={() => setPage(p => p - 1)} style={{ padding: '6px 12px', border: '1px solid #e5e7eb', borderRadius: 7, background: '#fff', cursor: page <= 1 ? 'default' : 'pointer', fontSize: 12.5, opacity: page <= 1 ? 0.5 : 1, fontFamily: "'Inter', sans-serif" }}>← Anterior</button>
                  <span style={{ fontSize: 12.5, color: '#6b7280', alignSelf: 'center' }}>Página {page} de {gastosPages}</span>
                  <button disabled={page >= gastosPages} onClick={() => setPage(p => p + 1)} style={{ padding: '6px 12px', border: '1px solid #e5e7eb', borderRadius: 7, background: '#fff', cursor: page >= gastosPages ? 'default' : 'pointer', fontSize: 12.5, opacity: page >= gastosPages ? 0.5 : 1, fontFamily: "'Inter', sans-serif" }}>Siguiente →</button>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: '#fff', borderRadius: 14, width: 480, maxHeight: '90vh', overflowY: 'auto',
        ...
        <div style={{ padding: '18px 22px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontWeight: 700, fontSize: 15, color: '#111' }}>Registrar gasto</span>
        ...
        <form onSubmit={handleSubmit} style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 4 }}>
            <button type="button" onClick={onClose} style={{ padding: '8px 16px', border: '1px solid #e5e7eb', borderRadius: 7, background: '#fff', cursor: 'pointer', fontSize: 13.5, fontFamily: "'Inter', sans-serif" }}>Cancelar</button>
