...
const ESTADO_CFG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  resolved:    { label: 'Resuelto',     color: 'var(--text-muted)', bg: '#f3f4f6' },
  false_alarm: { label: 'Falsa alarma', color: 'var(--text-faint)', bg: '#f3f4f6' },
};
...
export default function SosAlertPanel({ alertId, onClose, onUpdated }: Props) {
  ...
  if (loading) return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', zIndex: 1040 }} />
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 560, background: 'var(--bg-surface)', zIndex: 1050, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Spinner size={36} /></div>
    </>
  );
  if (error) return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', zIndex: 1040 }} />
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 560, background: 'var(--bg-surface)', zIndex: 1050, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ErrorState onRetry={load} /></div>
    </>
  );
  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', zIndex: 1040 }} />
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 560, background: 'var(--bg-surface)', zIndex: 1050, display: 'flex', flexDirection: 'column', boxShadow: '-4px 0 24px rgba(0,0,0,0.12)', fontFamily: "'Inter', sans-serif", overflowY: 'auto' }}>

        {/* Header */}
        <div style={{ position: 'sticky', top: 0, background: 'var(--bg-surface)', zIndex: 10, borderBottom: '1px solid var(--border)' }}>
          ...
          <div style={{ padding: '14px 18px' }}>
            ...
            {alert.nombre_animal && <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-primary)', marginTop: 6 }}>{alert.nombre_animal}</div>}
            <div style={{ fontSize: 12, color: 'var(--text-faint)', marginTop: 2 }}>{alert.codigo_referencia} · {formatDateTime(alert.created_at)}</div>
          </div>
        </div>
        ...
        <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          {/* Animal info */}
          <div style={{ background: 'var(--bg-subtle)', borderRadius: 10, padding: '12px 14px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '7px 16px' }}>
            {[
              ...
              <div key={f.label}>
                ...
                <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', textTransform: 'capitalize' }}>{f.val}</div>
              </div>
            ))}
          </div>
          ...
          {/* Ubicación */}
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 4 }}>Ubicación</div>
            <div style={{ fontSize: 13.5, color: 'var(--text-primary)', marginBottom: 4 }}>📍 {alert.ubicacion_descripcion || '—'}</div>
          ...
          {can('adopciones:manage') && (
            <div style={{ background: '#fef9c3', borderRadius: 10, padding: '12px 14px' }}>
              ...
              {alert.reportero_nombre && <div style={{ fontSize: 13.5, color: 'var(--text-primary)' }}>👤 {alert.reportero_nombre}</div>}
              {alert.reportero_telefono && <div style={{ fontSize: 13.5, color: 'var(--text-primary)' }}>📞 {alert.reportero_telefono}</div>}
              {alert.reportero_email && <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>✉️ {alert.reportero_email}</div>}
            </div>
          )}
          ...
          {alert.coincidencias && alert.coincidencias.length > 0 && (
            <div>
              <div style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--text-secondary)', marginBottom: 8 }}>🤖 Posibles coincidencias</div>
              <div style={{ display: 'flex', gap: 10, overflowX: 'auto' }}>
                {alert.coincidencias.map(c => (
                  <div key={c.id} style={{ flexShrink: 0, width: 100, textAlign: 'center' }}>
                    <div style={{ width: 80, height: 80, borderRadius: 10, overflow: 'hidden', background: 'var(--bg-subtle-2)', margin: '0 auto 4px' }}>
                    ...
                    <div style={{ fontSize: 10, color: 'var(--text-faint)' }}>{c.codigo_referencia}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {alert.updates && alert.updates.length > 0 && (
            <div>
              <div style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--text-secondary)', marginBottom: 8 }}>Actualizaciones</div>
              {alert.updates.map(u => (
                <div key={u.id} style={{ background: '#f0fdf4', borderRadius: 8, padding: '8px 12px', marginBottom: 6, fontSize: 13, borderLeft: '3px solid #16a34a' }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 3 }}>{formatDateTime(u.created_at)}{u.autor && ` · ${u.autor}`}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {/* Acciones */}
          {can('animales:update') && (
            <div style={{ background: 'var(--bg-subtle)', borderRadius: 10, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ fontWeight: 600, fontSize: 13.5, color: 'var(--text-primary)', marginBottom: 2 }}>Acciones</div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <select value={nuevoEstado} onChange={e => setNuevoEstado(e.target.value)} style={{ flex: 1, padding: '7px 10px', border: '1px solid var(--border)', borderRadius: 7, fontSize: 13, fontFamily: "'Inter', sans-serif", outline: 'none' }}>
                ...
                <button onClick={handleUpdateEstado} disabled={saving || nuevoEstado === alert.estado} style={{ padding: '7px 14px', background: nuevoEstado !== alert.estado ? '#374151' : 'var(--border)', color: nuevoEstado !== alert.estado ? '#fff' : '#9ca3af', border: 'none', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: "'Inter', sans-serif" }}>
                  {saving ? '...' : 'Cambiar'}
                </button>
              </div>
              <form onSubmit={handleAddUpdate} style={{ display: 'flex', gap: 8 }}>
                <input value={update} onChange={e => setUpdate(e.target.value)} placeholder="Añadir actualización..." style={{ flex: 1, padding: '7px 10px', border: '1px solid var(--border)', borderRadius: 7, fontSize: 13, fontFamily: "'Inter', sans-serif", outline: 'none' }} />
      ...
      {showConvertirModal && (
        <>
          <div onClick={() => setShowConvertirModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1060 }} />
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: 'var(--bg-surface)', borderRadius: 14, width: 400, padding: 24, zIndex: 1070, textAlign: 'center', fontFamily: "'Inter', sans-serif" }}>
            <div style={{ fontSize: 48, marginBottom: 10 }}>🐾</div>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 8 }}>Convertir en rescate</div>
            <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>Se creará una ficha de animal con los datos del aviso y estado "En evaluación". El aviso quedará marcado como rescatado.</div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 10 }}>
              <button onClick={() => setShowConvertirModal(false)} style={{ padding: '9px 18px', border: '1px solid var(--border)', borderRadius: 8, background: 'var(--bg-surface)', cursor: 'pointer', fontFamily: "'Inter', sans-serif" }}>Cancelar</button>
