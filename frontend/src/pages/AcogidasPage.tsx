...
const KARMA_LEVEL = (pts: number) => {
  ...
  if (pts >= 100)  return { emoji: '🥈', color: '#6b7280', label: 'Plata' };
...
export default function AcogidasPage() {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: '#f9fafb', minHeight: '100vh' }}>
      ...
      {/* Sub-nav */}
      <div style={{ padding: '0 24px', borderBottom: '1px solid #e5e7eb', background: '#fff', display: 'flex', alignItems: 'center' }}>
        {([
        ...
        <div style={{ padding: '12px 24px', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: '1 1 200px', maxWidth: 260 }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar familia..."
              style={{ width: '100%', padding: '7px 12px 7px 32px', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 13.5, outline: 'none', fontFamily: "'Inter', sans-serif", boxSizing: 'border-box' }} />
          </div>
          ...
          familiasFiltradas.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16, marginTop: 8 }}>
              {familiasFiltradas.map(f => {
                ...
                return (
                  <div key={f.id} style={{ background: '#fff', borderRadius: 14, border: '1px solid #e5e7eb', padding: '18px 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 700, fontSize: 14, color: '#111', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.nombre}</div>
                        <div style={{ fontSize: 12, color: '#9ca3af' }}>{f.zona || f.ciudad || '—'}</div>
                    ...
                    {/* Slots */}
                    <div>
                      <div style={{ fontSize: 11.5, color: '#9ca3af', marginBottom: 5 }}>Capacidad: {f.animales_actuales}/{f.max_animales}</div>
                      <div style={{ display: 'flex', gap: 4 }}>
                        {Array.from({ length: f.max_animales }, (_, i) => (
                          <div key={i} style={{ width: 12, height: 12, borderRadius: '50%', background: i < f.animales_actuales ? '#16a34a' : '#e5e7eb' }} />
                        ))}
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 2 }}>
                      <button onClick={() => setSelectedFamiliaId(f.id)} style={{ flex: 1, padding: '7px 0', border: '1px solid #e5e7eb', borderRadius: 7, background: '#fff', fontSize: 12.5, cursor: 'pointer', fontWeight: 500, fontFamily: "'Inter', sans-serif" }}>Ver perfil</button>
                      {can('animales:update') && (
                        <button onClick={() => setAsignarFamiliaId(f.id)} disabled={slotsLibres === 0} style={{
                          flex: 1, padding: '7px 0', border: 'none', borderRadius: 7,
                          background: slotsLibres > 0 ? '#16a34a' : '#e5e7eb',
                          color: slotsLibres > 0 ? '#fff' : '#9ca3af',
                    ...
                    {activas.map(a => {
                      ...
                      return (
                        <tr key={a.id} style={{ borderBottom: '1px solid #f9fafb' }}>
                          <td style={{ padding: '12px 14px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                              ...
                              <div>
                                ...
                                <div style={{ fontSize: 11.5, color: '#9ca3af', textTransform: 'capitalize' }}>{a.animal_especie} · {a.animal_raza}</div>
                              </div>
                            </div>
                          </td>
                          <td style={{ padding: '12px 14px' }}>
                            <div style={{ fontSize: 13.5, fontWeight: 500 }}>{a.familia_nombre}</div>
                            <div style={{ fontSize: 12, color: '#9ca3af' }}>{a.familia_zona}</div>
                          </td>
                          <td style={{ padding: '12px 14px', fontSize: 12.5, color: '#6b7280' }}>{formatDate(a.iniciada_at)}</td>
                          <td style={{ padding: '12px 14px' }}>
                          ...
                          <td style={{ padding: '12px 14px', fontSize: 12.5, color: a.ultimo_contacto ? '#374151' : '#dc2626' }}>
                            ...
                            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                              {can('animales:read') && (
                                <button onClick={() => setContactoData({ assignmentId: a.id, animalNombre: a.animal_nombre || '', familiaNombre: a.familia_nombre || '' })}
                                  style={{ padding: '4px 9px', border: '1px solid #e5e7eb', borderRadius: 6, background: '#fff', fontSize: 11.5, cursor: 'pointer', fontFamily: "'Inter', sans-serif", whiteSpace: 'nowrap' }}>
                                  📞 Contacto
                                </button>
                              )}
                              {can('animales:update') && (
                                <button onClick={() => setFinalizarData({ assignmentId: a.id, animalNombre: a.animal_nombre || '', familiaNombre: a.familia_nombre || '' })}
                                  style={{ padding: '4px 9px', border: '1px solid #fecaca', borderRadius: 6, background: '#fff', fontSize: 11.5, cursor: 'pointer', color: '#dc2626', fontFamily: "'Inter', sans-serif", whiteSpace: 'nowrap' }}>
                                  Finalizar
                                </button>
                              )}
                            </div>
                ...
                <tbody>
                  {historial.map(h => (
                    <tr key={h.id} style={{ borderBottom: '1px solid #f9fafb' }}>
                      ...
                      <td style={{ padding: '10px 14px', fontSize: 13, color: '#374151' }}>{h.familia_nombre}</td>
                      <td style={{ padding: '10px 14px', fontSize: 12.5, color: '#6b7280' }}>{formatDate(h.iniciada_at)}</td>
                      <td style={{ padding: '10px 14px', fontSize: 12.5, color: '#6b7280' }}>{formatDate(h.finalizada_at)}</td>
                      <td style={{ padding: '10px 14px', fontSize: 12.5, color: '#374151' }}>{Number(h.dias_totales) || '—'}d</td>
